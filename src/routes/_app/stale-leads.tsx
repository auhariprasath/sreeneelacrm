    ...
    <div className="space-y-4">
      <div>
        <h1 className="text-xl font-semibold flex items-center gap-2">
          <AlertTriangle className="h-5 w-5 text-warning" /> Stale leads
        </h1>
          ...
          <div key={r.id} className="p-3 flex items-center justify-between gap-2">
            <Link to="/leads/$leadId" params={{ leadId: r.id }} className="min-w-0 flex-1">
              ...
              <div className="text-xs text-muted-foreground truncate">
                ...
                <span className="text-warning font-medium">+{r.days_overdue}d over {r.threshold}d limit</span>
              </div>
            </Link>
            <a href={`tel:+91${r.phone.replace(/\D/g, "")}`}
              className="inline-flex items-center gap-1 h-9 px-3 rounded-md bg-success text-white text-xs">
